...
class ApiController extends Controller
{
                ...
                if(isset($item['fdData'])){
                    ...
                    if($item['type'] == "LH" || $item['type'] == "PD" || $item['type'] == "G" || $item['type'] == "W"){
                        ...
                        if($item['type'] == "W"){
                            ...
                            echo $item['type']."3 "."dd=".$fdData330->dd." done 330-2 <p>";
                        }else{
                            Sheerdata::updateOrInsert(
                                ['dd' => $fdData330['dd'], 'type' => $item['type']."3"],
                                [
                            ...
                            echo $item['type']."3"." dd=".$fdData330['dd']." done 330-1 <p>";
                        }
    ...
    public function saveLiveDB($date){
        ...
        $final_array = [
            ...
            [
                "type"=> "PD3",
                "fdData"=>!isset($main10_final->P3->fdData330) ? null : $main10_final->P3->fdData330,
                "jpData"=>!isset($main10_final->P3->jpData330) ? null : $main10_final->P3->jpData330,
                // "fdData"=>!isset($main9_final['PT15:30']) ? null : $main9_final['PT15:30'],
                // "jpData"=>!isset($main7_final["N63"]) ? null : $main7_final["N63"],
            ],
            [
                "type"=> "LH3",
                "fdData"=>!isset($main10_final->L3->fdData330) ? null : $main10_final->L3->fdData330,
                "jpData"=>!isset($main10_final->L3->jpData330) ? null : $main10_final->L3->jpData330,
            ...
            [
                "type"=> "W3",
                "fdData"=>!isset($main10_final->W->fdData330) ? null : $main10_final->W->fdData330,
                "jpData"=>!isset($main10_final->W->jpData330) ? null : $main10_final->W->jpData330,
            ],
        ];
        foreach ($final_array as $key => $value) {
            if(isset($value["fdData"])){
    ...
    public function getTMainByDateV1_4_0($date){
        date_default_timezone_set('Asia/Kuala_Lumpur');
        $today = date("Y-m-d");
        $status = "";
        //bydate
        ...
        //is Live
        $chpst = curl_init("https://kweelohstudio.com/api/allresults2/".$date);
        curl_setopt($chpst, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($chpst, CURLOPT_TIMEOUT, 3);
        curl_setopt($chpst, CURLOPT_CONNECTTIMEOUT, 3);
        $respst = curl_exec($chpst);
    ...
    public function getTMainByDateV1_3_0($date){
        date_default_timezone_set('Asia/Kuala_Lumpur');
        $today = date("Y-m-d");
        $status = "";
        //live
